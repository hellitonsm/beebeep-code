...
CUSTOM_NAME="${PACKAGE_NAME}_${VERSION}-${PKG_REVISION}${VERSION_SEPARATOR}debian${DEBIAN_VERSION_NUM}${VERSION_SEPARATOR}${ARCH}.deb"
CUSTOM_BUILDINFO="${PACKAGE_NAME}_${VERSION}-${PKG_REVISION}${VERSION_SEPARATOR}debian${DEBIAN_VERSION_NUM}${VERSION_SEPARATOR}${ARCH}.buildinfo"
CUSTOM_CHANGES="${PACKAGE_NAME}_${VERSION}-${PKG_REVISION}${VERSION_SEPARATOR}debian${DEBIAN_VERSION_NUM}${VERSION_SEPARATOR}${ARCH}.changes"

# Rename the files inside the build directory if they exist
cd "$BUILD_DIR"

if [ -f "${STANDARD_NAME}.deb" ]; then
    mv "${STANDARD_NAME}.deb" "$CUSTOM_NAME"
    echo "Renamed package to: $CUSTOM_NAME"
fi
